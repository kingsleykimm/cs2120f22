...
I think what proof of negation means is that you want to prove ¬P is true, so in that context, 
if you assume P is true, in the context of ¬P, which by itself already means that P → false, then that 
is a contadiction, meaning that this gives a proof of false so ¬P must be true.
-/
