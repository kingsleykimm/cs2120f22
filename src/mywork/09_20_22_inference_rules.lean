...
prove X /or Y; for then you will need only to apply the or_elim
...
BASICALLY, WHENEVER FALSE IS TRUE, EVERYTHING IS TRUE
-/
...
-/
/-
To prove X implies Y is true, you need to first assume that X is true, then in that context prove that Y is true.
...
--def p2 : Prop := false → true
/-
Steps: In english language, the proposition means if false is true, then true is true
First need to assume that f is true, then can use two different methods:
true.intro or false.elim
-/
example : p2 := 
begin
unfold p2,
...
Notes: Case tactic == apply false.elim, since case analysis will
find all the cases of a proof of false, but there are 0 ways false can be true
so there are no cases to be true
-/
end
