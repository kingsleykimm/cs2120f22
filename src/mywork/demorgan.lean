...
apply iff.intro _ _,
assume left,
--excluded middle takes a proposition and returns a proof of P ∨ ¬P 
cases (classical.em A) with a na,
cases (classical.em B) with b nb,

--Case A: true, B: True
let ab := and.intro a b,
--contradiction
let f := left ab,
apply false.elim f,
...
--Two cases with A false, B is true, but 
apply or.inl na,
--this statement satisfies it since or.intro satisfies both

assume right,
cases right with na nb, --whenever you see negation, need to prove that the thing that it is not implies false
-- in order to find something → something, use assume for arrow elimination
assume nab,
...
-- apply false.elim f,
--OR:
apply na (and.elim_left nab), --apply is the first term ONTO the second

assume nab,
-- apply nb (and.elim_right nab),
--OR:
cases nab with a b,
apply false.elim (nb b),
end


def dm2 := ∀ (A B : Prop), ¬(A ∨ B) ↔ ¬A ∧ ¬B
example : dm2 :=
begin
unfold dm2,
assume A B,
...
--case where A is true (or.elim)
assume a,
let aorb := or.intro_left B a, --for or.intro_left, the first argument needs to be the proposition you're not trying to prove
apply false.elim (left aorb),

--case where A is false
assume na,
apply or.elim (classical.em B),
assume b,
let aorb : A ∨ B := or.inr b, --another possible way of using or intro when it is clear which one you're using
contradiction, --use that when it is clear there is a contradiction between 
end
